...
export function createExamplePath(
  ...
  return pipe(
    ...
    E.bind('queryData', ({ pathData }) =>
      generateTemplateAndValuesForQueryParams(pathData.template, operation),
    ),
    E.map(({ pathData, queryData }) =>
      URI.expand(
        queryData.template,
        transformValues({ ...pathData.values, ...queryData.values }),
      ),
    ),
  );
}
        ...
        case HttpParamStyles.PipeDelimited:
          return pipe(
            ...
            E.fromPredicate(
              ...
              () =>
                new Error(
                  'Pipe delimited style is only applicable to array parameter',
                ),
            ),
            E.map((v) =>
              serializeWithPipeDelimitedStyle(spec.name, v, spec.explode),
            ),
          );
        ...
        case HttpParamStyles.SpaceDelimited:
          return pipe(
            ...
            E.fromPredicate(
              ...
              () =>
                new Error(
                  'Space delimited style is only applicable to array parameter',
                ),
            ),
            E.map((v) =>
              serializeWithSpaceDelimitedStyle(spec.name, v, spec.explode),
            ),
          );
...
function generateParamValues(
  specs: IHttpParam[],
): E.Either<Error, Dictionary<unknown>> {
  return pipe(
    specs,
...
function generateTemplateAndValuesForQueryParams(
  template: string,
  operation: IHttpOperation,
) {
...
function createPathUriTemplate(
  inputPath: string,
  ...
  return pipe(
    ...
    E.traverseArray((spec) =>
      pipe(
        createParamUriTemplate(
          spec.name,
          spec.style || HttpParamStyles.Simple,
          spec.explode || false,
        ),
    ...
    E.map((values) =>
      values.reduce(
        (acc, current) => acc.replace(`{${current.name}}`, current.param),
        inputPath,
      ),
    ),
  );
...
function createParamUriTemplate(
  name: string,
...
function createQueryUriTemplate(path: string, specs: IHttpQueryParam[]) {
  ...
  const formSpecs = specs
    .filter(
      (spec) => (spec.style || HttpParamStyles.Form) === HttpParamStyles.Form,
    )
    .map((spec) => {
  ...
  if (restParams) {
    path += `${
      formExplodedParams || formImplodedParams ? '&' : '?'
    }${restParams}`;
  }
